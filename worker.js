export default {
  async fetch(request) {
    ...
    async function sendDataService(targetName, value, noSpaces) {
    ...
    async function sendButtonCode(code) {
      return sendDataService("Button", String(code), false);
    }
    ...
    if (url.pathname === "/settime") {
      const hh = clamp(parseIntSafe(url.searchParams.get("hh"), -1), 0, 23);
      const mm = clamp(parseIntSafe(url.searchParams.get("mm"), -1), 0, 59);

      if (hh < 0 || mm < 0) {
        return makeJsonResponse({
          ok: false,
          error: "Použi parametre hh a mm, napr. /settime?hh=16&mm=45"
        }, 400);
      }
